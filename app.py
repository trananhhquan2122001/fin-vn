...
# =====================================================================
# [SECTION 20] - KẾT NỐI DỮ LIỆU REALTIME & BẢO VỆ API 3 TẦNG (ANTI-CRASH)
# =====================================================================
@st.cache_data(ttl=300)
def get_stock_data_bulletproof(symbol):
    """
    Hàm lấy dữ liệu chống sập: Luôn trả về đúng định dạng tuple (không bao giờ trả về None)
    để đảm bảo giao diện [SECTION 200] và [SECTION 250] không bao giờ bị biến mất.
    """
    sources = ['VCI', 'TCBS', 'MSN']
    
    # Giá trị mặc định an toàn khi tất cả API đều lỗi
    price = 0.0
    market_cap = "N/A"
    pe = "N/A"
    pb = "N/A"
    ...
    for src in sources:
        try:
            ...
            # Nếu lấy thành công ít nhất Giá hoặc BCTC thì xác nhận nguồn chạy tốt
...
# =====================================================================
# [SECTION 120] - ĐỊNH GIÁ HYBRID (GRAHAM + ML) - ĐÃ CÓ Ở SECTION 110
# =====================================================================
# (hàm hybrid_valuation_ensemble đã được định nghĩa ở trên)
...
# =====================================================================
# [SECTION 200] - GIAO DIỆN PHẦN ĐẦU & THANH TÌM KIẾM (UI HEADER & SEARCH)
# =====================================================================
def main():
    ...
    # Gọi hàm an toàn (luôn trả về kết quả, không bao giờ gây crash)
    price, market_cap, pe, pb, roe, df_inc, df_bs, active_src, err = get_stock_data_bulletproof(ticker_input)

    # Hiển thị trạng thái máy chủ (không làm mất màn hình)
    if active_src != "Không kết nối":
        st.caption(f"🟢 Trạng thái dữ liệu: Đã kết nối qua máy chủ **{active_src}**")
    else:
        st.warning(f"⚠️ Máy chủ dữ liệu chứng khoán đang bận hoặc nghẽn mạng tạm thời đối với mã **{ticker_input}**. Dưới đây là khung giao diện sẵn sàng khi dữ liệu phục hồi.")

    # GIAO DIỆN LUÔN HIỂN THỊ DÙ CÓ DỮ LIỆU HAY KHÔNG
    st.subheader("📊 Thông Tin Nhanh")
    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric("Giá Hiện Tại", f"{price:,.0f} VNĐ" if price > 0 else "Đang tải...")
    ...
    # =====================================================================
    # [SECTION 250] - HIỂN THỊ CHI TIẾT TABS (8 TABS)
    # =====================================================================

    # Nếu chưa có dữ liệu BCTC, hiển thị thông báo nhưng vẫn giữ các tab
    if df_inc.empty or df_bs.empty:
        st.warning("Dữ liệu BCTC chưa được tải. Một số chức năng sẽ bị hạn chế.")
        # Tạo DataFrame rỗng để các hàm không bị lỗi
        df = pd.DataFrame()
        sector = "general"
        row_data = {}
    else:
        ...
        data = {
            ...
            'Vốn hóa': 0,   # sẽ lấy từ market_cap nếu có
            'Giá hiện tại': price,
            'P/E': 0,
            'P/B': 0,
            'ROE': 0,
    ...
    # Lưu vào session_state cho các hàm khác
    st.session_state.df = df
    st.session_state.ticker_col = 'Mã CP'
    st.session_state.selected_ticker = ticker_input

    # Trích xuất chỉ số (nếu có dữ liệu)
    if not df.empty:
    ...
    # TÍN HIỆU MUA (nếu có dữ liệu)
    st.markdown("### 🚦 TÍN HIỆU ĐẦU TƯ - BỘ LỌC 3 TẦNG")
    if not df.empty and (has_eps or eps > 0):
    ...
    with tab_search:
        ...
        if search_ticker:
            _, _, _, _, _, inc2, bs2, _, _ = get_stock_data_bulletproof(search_ticker)
            if not inc2.empty:
