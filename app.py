...
try:
    ...
    try:
        ...
        Vnstock3 = None
        VNSTOCK3_AVAILABLE = False

# =========================================================
# [SECTION 20] - KẾT NỐI DỮ LIỆU REALTIME & BẢO VỆ API 3 TẦNG
# =========================================================
SAFE_SOURCES = ["VCI", "TCBS", "MSN"]
SAFE_EMPTY_DF = pd.DataFrame()
SAFE_DEFAULTS = {
    "ticker": "",
    "price": 0.0,
    "market_cap": 0.0,
...
def _safe_frame(value):
    """Luôn biến kết quả API thành DataFrame an toàn."""
    try:
        if isinstance(value, pd.DataFrame):
            return fix_duplicate_columns(value.copy())
        if isinstance(value, pd.Series):
            return fix_duplicate_columns(value.to_frame().T)
        if isinstance(value, list):
            return fix_duplicate_columns(pd.DataFrame(value))
        if isinstance(value, dict):
            if "data" in value and isinstance(value["data"], list):
                return fix_duplicate_columns(pd.DataFrame(value["data"]))
            return fix_duplicate_columns(pd.DataFrame([value]))
    except Exception:
        pass
    return pd.DataFrame()
...
def _safe_number(value):
    try:
        return float(to_float_scalar(value))
    except Exception:
        return 0.0

def _normalize_realtime_price(value):
    """Chuẩn hóa giá: một số nguồn trả giá theo nghìn đồng."""
    price = _safe_number(value)
    if 0 < price < 1000:
        price *= 1000
    return price

def _row_keyword_value(df, keywords):
    """Quét keyword chữ thường, loại bỏ N/A và giá trị rỗng."""
    try:
...
def _first_numeric_from_frame(df, keywords):
    return _row_keyword_value(df, keywords)

def _try_vnstock_source(ticker, source):
    """
    Tầng dữ liệu 1: vnstock3/vnstock.
    Mọi lỗi của source chỉ được ghi nhận nội bộ và chuyển sang source kế tiếp.
    """
    ...
    if not VNSTOCK3_AVAILABLE:
        result["error"] = "vnstock3/vnstock không khả dụng"
        return result, pd.DataFrame(), pd.DataFrame(), pd.DataFrame()

    try:
        stock = Vnstock3().stock(symbol=ticker, source=source)

        # Giá realtime / gần realtime.
        quote_df = pd.DataFrame()
        for method_name, kwargs in [
            ("history", {"length": "5", "interval": "1D"}),
            ("history", {"length": "1", "interval": "1D"}),
        ]:
            try:
                method = getattr(stock.quote, method_name)
                quote_df = _safe_frame(method(**kwargs))
        ...
        if not quote_df.empty:
            price_col_candidates = ["close", "price", "last", "close_price", "giá"]
            for col in quote_df.columns:
                if str(col).lower() in [x.lower() for x in price_col_candidates]:
                    values = pd.to_numeric(quote_df[col], errors="coerce").dropna()
                    if not values.empty:
                        result["price"] = _normalize_realtime_price(values.iloc[-1])
                        break
        ...
        try:
            income_df = _safe_frame(
                stock.finance.income_statement(period="year", lang="vi", dropna=True)
            )
        except Exception:
            try:
                income_df = _safe_frame(
                    stock.finance.income_statement(period="year", dropna=True)
                )
            except Exception:
                income_df = pd.DataFrame()

        try:
            balance_df = _safe_frame(
                stock.finance.balance_sheet(period="year", lang="vi", dropna=True)
            )
        except Exception:
            try:
                balance_df = _safe_frame(
                    stock.finance.balance_sheet(period="year", dropna=True)
                )
            except Exception:
                balance_df = pd.DataFrame()
        ...
        try:
            ratio_df = _safe_frame(
                stock.finance.ratio(period="year", lang="vi", dropna=True)
            )
        except Exception:
            try:
                ratio_df = _safe_frame(stock.finance.ratio(period="year", dropna=True))
            except Exception:
                ratio_df = pd.DataFrame()

        # Quét ratio theo keyword chữ thường.
        result["pe"] = _first_numeric_from_frame(ratio_df, ["pe", "p/e", "price to earnings"])
        result["pb"] = _first_numeric_from_frame(ratio_df, ["pb", "p/b", "price to book"])
        result["roe"] = _first_numeric_from_frame(ratio_df, ["roe", "return on equity"])
        result["market_cap"] = _first_numeric_from_frame(
            ratio_df, ["market cap", "market_cap", "vốn hóa"]
        )

        # Fallback từ BCTC nếu ratio không có.
        if result["roe"] == 0:
            result["roe"] = _first_numeric_from_frame(
                income_df, ["roe", "return on equity"]
            )

        eps = _first_numeric_from_frame(
            ratio_df, ["eps", "earnings per share", "lãi cơ bản"]
        )
        equity = _first_numeric_from_frame(
            balance_df, ["vốn chủ sở hữu", "total equity", "equity"]
        )
        shares = _first_numeric_from_frame(
            balance_df, ["shares outstanding", "cổ phiếu lưu hành", "shares"]
        )

        if result["price"] <= 0:
            result["price"] = _normalize_realtime_price(
                _first_numeric_from_frame(quote_df, ["close", "price", "last", "giá"])
            )

        if result["pe"] <= 0 and eps > 0 and result["price"] > 0:
            result["pe"] = result["price"] / eps

        if result["pb"] <= 0 and equity > 0 and shares > 0 and result["price"] > 0:
            bvps = equity / shares
            if bvps > 0:
                result["pb"] = result["price"] / bvps

        # Chỉ coi source là thành công nếu có ít nhất một dữ liệu hữu ích.
        useful = any(
            _safe_number(result.get(k, 0)) > 0
            for k in ["price", "pe", "pb", "roe", "market_cap"]
        ) or not income_df.empty or not balance_df.empty
        ...
        if useful:
            ...
            result["status"] = "empty"
            result["error"] = "Nguồn trả về dữ liệu rỗng"

        return result, income_df, balance_df, ratio_df

    except Exception as exc:
        result["error"] = f"{type(exc).__name__}: {str(exc)[:180]}"
        return result, pd.DataFrame(), pd.DataFrame(), pd.DataFrame()

def _safe_mock_bundle(ticker):
    """Tầng 3: dữ liệu dự phòng deterministic, chỉ dùng để giữ UI hoạt động."""
    try:
    ...
    income = pd.DataFrame([{
        ...
        "revenue": _safe_number(mock.get("revenue", 0)),
        "profit_after_tax": _safe_number(mock.get("profit", 0)),
        "ebit": _safe_number(mock.get("profit", 0)),
    }])

    balance = pd.DataFrame([{
        "ticker": ticker,
        "year": pd.Timestamp.now().year,
        "total_assets": max(_safe_number(mock.get("equity", 0)) * 2.0, 1.0),
        "total_liabilities": max(_safe_number(mock.get("equity", 0)), 0.0),
        "equity": _safe_number(mock.get("equity", 0)),
        "working_capital": _safe_number(mock.get("equity", 0)) * 0.25,
        "retained_earnings": _safe_number(mock.get("profit", 0)) * 2.0,
    }])

    ratios = pd.DataFrame([{
        "ticker": ticker,
        "pe": _safe_number(mock.get("pe", 0)),
        "pb": _safe_number(mock.get("pb", 0)),
        "roe": _safe_number(mock.get("roe", 0)),
        "market_cap": _safe_number(mock.get("market_cap", 0)),
    }])

    data = {
        **SAFE_DEFAULTS,
        **mock,
        "price": _normalize_realtime_price(mock.get("price", 0)),
        "market_cap": _safe_number(mock.get("market_cap", 0)),
        "pe": _safe_number(mock.get("pe", 0)),
        "pb": _safe_number(mock.get("pb", 0)),
        "roe": _safe_number(mock.get("roe", 0)),
    ...
    return data, income, balance, ratios

@st.cache_data(ttl=300, show_spinner=False)
def get_safe_realtime_bundle(ticker):
    """
    Hợp đồng an toàn:
    luôn trả về đúng tuple (data, income_df, balance_df, ratio_df, source_status).
    Không trả None và không để exception thoát ra UI.
    """
    ticker = str(ticker or "HPG").upper().strip()
    if not ticker:
        ticker = "HPG"

    errors = []

    # Tầng 1: lần lượt VCI -> TCBS -> MSN.
    for source in SAFE_SOURCES:
        try:
            data, income_df, balance_df, ratio_df = _try_vnstock_source(ticker, source)
            if data.get("status") == "ok":
                data["source"] = source
                status = {
                    "ok": True,
                    "source": source,
                    "message": f"🟢 Nguồn: {source}",
                    "errors": errors,
                }
                return data, income_df, balance_df, ratio_df, status
            errors.append(f"{source}: {data.get('error', 'không có dữ liệu')}")
        except Exception as exc:
            errors.append(f"{source}: {type(exc).__name__}")

    # Tầng 2/3: dùng dữ liệu fallback cục bộ để UI luôn tồn tại.
    try:
        data, income_df, balance_df, ratio_df = _safe_mock_bundle(ticker)
        status = {
            "ok": False,
            "source": "FALLBACK",
            "message": "🟠 Fallback cục bộ: API đang lỗi/nghẽn",
            "errors": errors,
        }
        return data, income_df, balance_df, ratio_df, status
    except Exception as exc:
        # Tuyệt đối không để None lọt ra ngoài.
        ...
        status = {
            "ok": False,
            "source": "OFFLINE",
            "message": "🔴 Offline safe mode",
            "errors": errors,
        }
        return data, pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), status

# =========================================================
# [SECTION 60] - BỘ HÀM TOÁN HỌC & ĐÁNH GIÁ SỨC KHỎE TÀI CHÍNH
# =========================================================
def _latest_value(df, keywords, default=0.0):
    frame = _safe_frame(df)
    if frame.empty:
        return float(default)

    # Ưu tiên cột chứa keyword.
    candidates = []
    for col in frame.columns:
...
def _previous_value(df, keywords, default=0.0):
    frame = _safe_frame(df)
    if frame.empty:
        return float(default)
    candidates = [
        col for col in frame.columns
        if any(str(k).lower() in str(col).lower() for k in keywords)
    ]
    for col in candidates:
        series = pd.to_numeric(frame[col], errors="coerce").dropna()
        if len(series) >= 2:
...
def calculate_piotroski_score(df_inc, df_bs):
    """
    Piotroski F-Score 0-9:
    1 ROA dương
    2 CFO dương nếu có dữ liệu
    3 ROA tăng
    4 Chất lượng lợi nhuận/CFO > NI nếu có dữ liệu
    5 Nợ giảm
    6 Thanh khoản cải thiện
    7 Không pha loãng
    8 Biên lợi nhuận gộp tăng
    9 Vòng quay tài sản tăng
    """
    score = 0
    inc = _safe_frame(df_inc)
    bs = _safe_frame(df_bs)

    ni = _latest_value(inc, ["net income", "profit after tax", "lợi nhuận sau thuế", "lnst", "profit"])
    ni_prev = _previous_value(inc, ["net income", "profit after tax", "lợi nhuận sau thuế", "lnst", "profit"])

    assets = _latest_value(bs, ["total assets", "tổng tài sản", "assets"])
    assets_prev = _previous_value(bs, ["total assets", "tổng tài sản", "assets"])

    roa = ni / assets if assets > 0 else 0.0
    roa_prev = ni_prev / assets_prev if assets_prev > 0 else 0.0
    if roa > 0:
        score += 1

    cfo = _latest_value(inc, ["cash flow from operating", "operating cash flow", "cfo", "lưu chuyển tiền thuần từ hoạt động kinh doanh"])
    if cfo > 0:
        score += 1
    elif inc.empty:
        pass

    if roa > roa_prev and assets_prev > 0:
        score += 1

    if cfo > ni and ni != 0:
        score += 1

    debt = _latest_value(bs, ["total liabilities", "tổng nợ", "liabilities", "debt"])
    debt_prev = _previous_value(bs, ["total liabilities", "tổng nợ", "liabilities", "debt"])
    if debt_prev > 0 and debt < debt_prev:
        score += 1

    current_assets = _latest_value(bs, ["current assets", "tài sản ngắn hạn"])
    current_liabilities = _latest_value(bs, ["current liabilities", "nợ ngắn hạn", "current debt"])
    current_assets_prev = _previous_value(bs, ["current assets", "tài sản ngắn hạn"])
    current_liabilities_prev = _previous_value(bs, ["current liabilities", "nợ ngắn hạn", "current debt"])
    cr = current_assets / current_liabilities if current_liabilities > 0 else 0.0
    cr_prev = current_assets_prev / current_liabilities_prev if current_liabilities_prev > 0 else 0.0
    if cr > cr_prev and current_liabilities_prev > 0:
        score += 1

    shares = _latest_value(bs, ["shares outstanding", "cổ phiếu lưu hành", "shares"])
    shares_prev = _previous_value(bs, ["shares outstanding", "cổ phiếu lưu hành", "shares"])
    if shares_prev <= 0 or shares <= shares_prev:
        score += 1

    revenue = _latest_value(inc, ["revenue", "doanh thu", "sales"])
    cogs = _latest_value(inc, ["cost of goods sold", "giá vốn hàng bán", "cost of sales"])
    revenue_prev = _previous_value(inc, ["revenue", "doanh thu", "sales"])
    cogs_prev = _previous_value(inc, ["cost of goods sold", "giá vốn hàng bán", "cost of sales"])
    gm = (revenue - cogs) / revenue if revenue > 0 else 0.0
    gm_prev = (revenue_prev - cogs_prev) / revenue_prev if revenue_prev > 0 else 0.0
    if revenue > 0 and revenue_prev > 0 and gm > gm_prev:
        score += 1

    asset_turnover = revenue / assets if assets > 0 else 0.0
    asset_turnover_prev = revenue_prev / assets_prev if assets_prev > 0 else 0.0
    if revenue > 0 and revenue_prev > 0 and asset_turnover > asset_turnover_prev:
        score += 1

    return int(max(0, min(9, score)))

def calculate_altman_z_score(df_bs, df_inc):
    """Altman Z-Score cho doanh nghiệp phi tài chính; dữ liệu thiếu => 0.0."""
    try:
        bs = _safe_frame(df_bs)
        inc = _safe_frame(df_inc)

        wc = _latest_value(bs, ["working capital", "vốn lưu động"])
        ta = _latest_value(bs, ["total assets", "tổng tài sản", "assets"])
        re = _latest_value(bs, ["retained earnings", "lợi nhuận giữ lại", "retained"])
        ebit = _latest_value(inc, ["ebit", "operating income", "lợi nhuận hoạt động", "lợi nhuận trước lãi vay"])
        market_cap = _latest_value(bs, ["market cap", "vốn hóa", "market capitalization"])
        liabilities = _latest_value(bs, ["total liabilities", "tổng nợ", "liabilities"])
        sales = _latest_value(inc, ["revenue", "doanh thu", "sales"])
        ...
        b = re / ta
        c = ebit / ta
        d = market_cap / liabilities if liabilities > 0 else 0.0
        e = sales / ta
...
# =========================================================
# [SECTION 70] - MÔ HÌNH ĐỊNH GIÁ DOANH NGHIỆP RẺ / ĐẮT
# =========================================================
def calculate_dcf_valuation(fcf, growth_rate, discount_rate, margin_of_safety):
    """DCF/Gordon đơn giản, trả về giá trị nội tại và giá mua an toàn."""
    fcf = _safe_number(fcf)
    g = _safe_number(growth_rate) / 100.0
    r = _safe_number(discount_rate) / 100.0
    mos = max(0.0, min(0.95, _safe_number(margin_of_safety) / 100.0))

    if fcf <= 0:
        return {
            "intrinsic_value": 0.0,
            "safe_buy_price": 0.0,
            "conclusion": "Không đủ FCF để định giá",
        }

    if r <= g:
        intrinsic = fcf * 15.0
    else:
        intrinsic = (fcf * (1.0 + g)) / (r - g)

    safe_price = intrinsic * (1.0 - mos)
    return {
        "intrinsic_value": float(max(0.0, intrinsic)),
        "safe_buy_price": float(max(0.0, safe_price)),
        "conclusion": "Định giá Rẻ" if safe_price > 0 else "Định giá Đắt",
    }

def calculate_graham_valuation(eps, g):
    """Benjamin Graham cải tiến: EPS * (8.5 + 2g) * 4.4 / Y."""
    eps = _safe_number(eps)
    growth = _safe_number(g)
    if eps <= 0:
        return 0.0
    # Y = 10% được dùng làm lợi suất trái phiếu tham chiếu mặc định.
    return float(max(0.0, eps * (8.5 + 2.0 * growth) * 4.4 / 10.0))

def valuation_conclusion(current_price, intrinsic_value, margin_of_safety=20.0):
    price = _safe_number(current_price)
    intrinsic = _safe_number(intrinsic_value)
    mos = _safe_number(margin_of_safety)
    if intrinsic <= 0 or price <= 0:
        return "Chưa đủ dữ liệu"
    safe_price = intrinsic * (1 - mos / 100.0)
    return "Cổ phiếu đang Định giá Rẻ" if price <= safe_price else "Cổ phiếu đang Định giá Đắt"

# =========================================================
# [SECTION 150] - PHÂN TÍCH TOÀN CẢNH BỨC TRANH DOANH NGHIỆP
# =========================================================
def _series_by_keywords(df, keywords):
    frame = _safe_frame(df)
    if frame.empty:
...
def generate_business_picture(df_inc, df_bs):
    inc = _safe_frame(df_inc)
    bs = _safe_frame(df_bs)
    ...
    for frame in [inc, bs]:
        for col in frame.columns:
            if str(col).lower() in {"year", "năm", "period", "report_date", "fiscal_year"}:
                try:
                    years.extend(frame[col].astype(str).tolist())
                except Exception:
                    pass
                break

    revenue = _series_by_keywords(inc, ["revenue", "doanh thu", "sales"])
    gross_profit = _series_by_keywords(inc, ["gross profit", "lợi nhuận gộp"])
    assets = _series_by_keywords(bs, ["total assets", "tổng tài sản", "assets"])
    equity = _series_by_keywords(bs, ["equity", "vốn chủ sở hữu", "vốn chủ"])
    liabilities = _series_by_keywords(bs, ["liabilities", "tổng nợ", "total debt"])

    n = max(len(revenue), len(gross_profit), len(assets), len(equity), len(liabilities))
    if n == 0:
        return pd.DataFrame(columns=[
            "Năm", "Biên LN gộp (%)", "Vòng quay tài sản", "Nợ/Vốn chủ sở hữu"
        ])

    def pad(series):
        values = series.tolist() if not series.empty else []
        values = [np.nan if pd.isna(x) else _safe_number(x) for x in values]
        return values + [np.nan] * (n - len(values))

    rev = pad(revenue)
    gp = pad(gross_profit)
    ta = pad(assets)
    eq = pad(equity)
    liab = pad(liabilities)

    if not years or len(years) < n:
        years = [f"Năm {i+1}" for i in range(n)]
    else:
        years = years[:n]
    ...
    output = pd.DataFrame({
        "Năm": years,
        "Biên LN gộp (%)": [
            (gp[i] / rev[i] * 100) if rev[i] not in (0, np.nan) and pd.notna(rev[i]) and pd.notna(gp[i]) else np.nan
            for i in range(n)
        ],
        "Vòng quay tài sản": [
            (rev[i] / ta[i]) if pd.notna(ta[i]) and ta[i] != 0 and pd.notna(rev[i]) else np.nan
            for i in range(n)
        ],
        "Nợ/Vốn chủ sở hữu": [
            (liab[i] / eq[i]) if pd.notna(eq[i]) and eq[i] != 0 and pd.notna(liab[i]) else np.nan
            for i in range(n)
        ],
    })
    return output
...
# =========================================================
def _safe_metric_value(value, suffix="", currency=False):
    try:
        number = _safe_number(value)
        if number <= 0:
            return "Đang tải…" if st.session_state.get("finex_loading", False) else "N/A"
        if currency:
            return format_currency_vn_advanced(number, per_share=True)
        return f"{number:,.2f}{suffix}"
    except Exception:
        return "N/A"

def _render_safe_header(ticker, status):
    st.markdown(
        """
        <div class="header-box">
            <div style="display:flex;flex-direction:column;">
                <div class="header-title">📊 FINEX VN <span>TERMINAL</span></div>
                <div class="header-sub">Safe UI • Anti-Crash • Multi-source Financial Analysis</div>
            </div>
        </div>
        """,
    ...
    if status.get("ok"):
        st.success(status.get("message", "🟢 Đã kết nối dữ liệu"))
    else:
        st.warning(status.get("message", "🟠 Đang sử dụng chế độ an toàn"))
        if status.get("errors"):
            with st.expander("Chi tiết lỗi API (không ảnh hưởng giao diện)"):
                for error in status["errors"]:
                    st.caption(f"• {error}")

    st.caption(f"Đang phân tích: **{ticker}**")

def _render_safe_metrics(data):
    cols = st.columns(5)
    cols[0].metric("Giá", _safe_metric_value(data.get("price"), currency=True))
    cap = _safe_number(data.get("market_cap"))
    cols[1].metric(
        "Vốn Hóa",
        format_currency_vn_advanced(cap) if cap > 0 else "N/A"
    )
    cols[2].metric("P/E", _safe_metric_value(data.get("pe")))
    cols[3].metric("P/B", _safe_metric_value(data.get("pb")))
    roe = _safe_number(data.get("roe"))
    cols[4].metric("ROE", f"{roe:.2f}%" if roe > 0 else "N/A")

# =========================================================
# [SECTION 250] - HIỂN THỊ CHI TIẾT TABS & BẢNG BCTC
# =========================================================
def _render_safe_diagnostic(ticker, data, income_df, balance_df):
    st.subheader(f"🛡️ Chẩn Đoán Sức Khỏe & Định Giá — {ticker}")

    piotroski = calculate_piotroski_score(income_df, balance_df)
    altman = calculate_altman_z_score(balance_df, income_df)

    eps = _first_numeric_from_frame(income_df, ["eps", "earnings per share", "lãi cơ bản"])
    if eps <= 0:
        eps = _safe_number(data.get("eps", 0))

    fcf = _first_numeric_from_frame(
        income_df,
        ["free cash flow", "free_cash_flow", "dòng tiền tự do", "fcf"],
    )
    if fcf <= 0:
        fcf = _safe_number(data.get("profit", 0)) * 0.7

    dcf = calculate_dcf_valuation(fcf, 10.0, 12.0, 25.0)
    graham = calculate_graham_valuation(eps, 10.0)
    current_price = _safe_number(data.get("price", 0))
    best_intrinsic = max(dcf["intrinsic_value"], graham)
    conclusion = valuation_conclusion(current_price, best_intrinsic, 20.0)

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Piotroski F-Score", f"{piotroski}/9")
    c2.metric("Altman Z-Score", f"{altman:.2f}" if altman else "N/A")
    c3.metric("Graham Value", format_currency_vn_advanced(graham, per_share=True) if graham else "N/A")
    c4.metric("DCF Value", format_currency_vn_advanced(dcf["intrinsic_value"], per_share=True) if dcf["intrinsic_value"] else "N/A")

    if "Rẻ" in conclusion:
        st.success(f"🟢 {conclusion}")
    elif "Đắt" in conclusion:
        st.error(f"🔴 {conclusion}")
    else:
        st.info(f"ℹ️ {conclusion}")

    st.markdown("#### 📋 Báo cáo định giá")
    valuation_df = pd.DataFrame([
        {
            "Mô hình": "DCF",
            "Giá trị nội tại": dcf["intrinsic_value"],
            "Giá mua an toàn": dcf["safe_buy_price"],
            "Kết luận": valuation_conclusion(current_price, dcf["intrinsic_value"], 20.0),
        },
        {
            "Mô hình": "Graham",
            "Giá trị nội tại": graham,
            "Giá mua an toàn": graham * 0.8 if graham > 0 else 0,
            "Kết luận": valuation_conclusion(current_price, graham, 20.0),
        },
    ])
    st.dataframe(valuation_df, use_container_width=True, hide_index=True)

def _render_safe_business_picture(income_df, balance_df):
    st.subheader("📈 Bức Tranh Doanh Nghiệp")
    picture = generate_business_picture(income_df, balance_df)
    if picture.empty:
        st.info("Chưa đủ dữ liệu để dựng bức tranh tài chính.")
        return
    st.dataframe(picture, use_container_width=True, hide_index=True)

    if len(picture) >= 2:
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=picture["Năm"],
            y=picture["Biên LN gộp (%)"],
            mode="lines+markers",
            name="Biên LN gộp (%)",
        ))
        fig.add_trace(go.Scatter(
            x=picture["Năm"],
            y=picture["Vòng quay tài sản"],
            mode="lines+markers",
        ...
        fig.update_layout(
            ...
            height=420,
            yaxis=dict(title="Biên LN gộp (%)"),
            yaxis2=dict(title="Vòng quay tài sản", overlaying="y", side="right"),
        )
        st.plotly_chart(fig, use_container_width=True)

def _render_safe_raw_statements(income_df, balance_df):
    st.subheader("📑 BCTC Thô")
    tab_inc, tab_bs = st.tabs(["Kết quả Kinh doanh", "Cân đối Kế toán"])
    with tab_inc:
        if income_df.empty:
            st.info("Không có BCKQKD từ nguồn hiện tại.")
        else:
            st.dataframe(income_df, use_container_width=True, hide_index=True)
    with tab_bs:
        if balance_df.empty:
            st.info("Không có BCĐKT từ nguồn hiện tại.")
        else:
            st.dataframe(balance_df, use_container_width=True, hide_index=True)

def _render_legacy_bridge():
    st.markdown("---")
    with st.expander("🧩 FINEX Legacy Terminal — giữ nguyên giao diện và thuật toán cũ"):
        st.caption(
            "Toàn bộ code gốc được giữ trong file và được cô lập khỏi Safe UI. "
            "Mở rộng này không cho lỗi API làm mất giao diện mới."
        )
        if st.button("Mở Legacy Terminal", key="open_legacy_terminal"):
            try:
                legacy_main()
            except Exception as exc:
                st.error(f"Legacy Terminal gặp lỗi nhưng Safe UI vẫn hoạt động: {exc}")

# =========================================================
# [SECTION 260] - MAIN SAFE TERMINAL
# =========================================================
def main():
    try:
        st.session_state["finex_loading"] = True

        ticker_input = st.text_input(
            "🔍 Nhập mã CP:",
            value="HPG",
            key="finex_ticker_input",
        )
        ticker = str(ticker_input or "HPG").upper().strip() or "HPG"

        data, income_df, balance_df, ratio_df, status = get_safe_realtime_bundle(ticker)
        st.session_state["finex_loading"] = False

        _render_safe_header(ticker, status)
        _render_safe_metrics(data)
        ...
        with tabs[0]:
            _render_safe_diagnostic(ticker, data, income_df, balance_df)

        with tabs[1]:
            _render_safe_business_picture(income_df, balance_df)

        with tabs[2]:
            _render_safe_raw_statements(income_df, balance_df)

        _render_legacy_bridge()

        st.markdown("---")
        st.caption(
            "FINEX VN Terminal • UI Safety Guard: mọi lỗi 404, timeout, "
            "exception hoặc nguồn dữ liệu rỗng đều được chuyển sang fallback, "
            "không dùng st.stop() để làm trắng trang."
        )

    except Exception as exc:
        # Lớp bảo vệ cuối cùng: ngay cả lỗi bất ngờ trong render cũng không
        # biến ứng dụng thành một trang trắng.
        st.session_state["finex_loading"] = False
        st.error("⚠️ FINEX Safe UI đã kích hoạt chế độ bảo vệ.")
        st.warning(f"Chi tiết kỹ thuật: {type(exc).__name__}: {str(exc)[:300]}")
        fallback = {
            **SAFE_DEFAULTS,
            "ticker": str(st.session_state.get("finex_ticker_input", "HPG")).upper().strip(),
            "source": "OFFLINE",
        }
        _render_safe_metrics(fallback)
        st.info(
            "Giao diện vẫn hoạt động. Hãy thử mã khác hoặc tải lại ứng dụng; "
            "lỗi kết nối bên ngoài không được phép làm mất UI."
        )
...
# =========================================================
# [LEGACY CODE] - TOÀN BỘ MÃ NGUỒN GỐC ĐƯỢC GIỮ LẠI
# =========================================================
import os
import numpy as np
import pandas as pd
...
# ============================================================================
# Legacy set_page_config removed: Safe SECTION 10 owns page configuration.
...
# Legacy entrypoint intentionally disabled; FINEX Safe Terminal is the active entrypoint.
